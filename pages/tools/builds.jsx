...
// The pre-community builds page lived at this path with ?c=<class>&b=<index> (an index into
// the curated data/builds.json list). Those builds were imported into the community system,
// so every old URL has an exact new home; the map is generated by
// utility/builds/generate-legacy-redirects.mjs. Old links still circulate (and Google soft-404s
// them), so redirect 1:1 - exact build when the index maps, the class page otherwise.
export const legacyRedirectTarget = (query, map = legacyRedirects) => {
  const c = typeof query?.c === 'string' ? query.c.toLowerCase() : '';
  const entry = map[c];
  if (!entry) return null;
  const slug = (typeof query?.b === 'string' && entry.builds?.[query.b]) || entry.class;
  return `/tools/builds/${slug}`;
};
...
const Builds = ({ initialBuilds }) => {
  ...
  useEffect(() => {
    ...
    const target = legacyRedirectTarget(router.query);
