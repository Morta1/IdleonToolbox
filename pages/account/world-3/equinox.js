...
import { notateNumber,cleanUnderscore, prefix } from '../../../utility/helpers';
...
const Equinox = () => {
    ...
    return (
        ...
        <Stack mb={1} direction={'row'} gap={{ xs: 1, md: 3 }} flexWrap={'wrap'}>
          <CardTitleAndValue title={'Speed'} value={Math.round(equinox.chargeRate) + "/hr"} width={'200px'} fontSize={40}/>
          <CardTitleAndValue textAlign={'center'} title={'Equinox Progression'} width={'400px'} >
                <Typography textAlign={'center'} >{notateNumber(equinox.currentCharge, 'Whole')} / {notateNumber(equinox.chargeRequired, 'Whole')}</Typography>
