...
const Construction = () => {
  ...
  return (
    <>
      <Typography variant={'h2'} textAlign={'center'} mb={3}>Construction</Typography>
      <Stack alignItems={'center'}>
        ...
        >
          ...
          <ToggleButton value="flaggy">
            Flaggy
          </ToggleButton>
          <ToggleButton value="classExp">
            Class Exp
          </ToggleButton>
        </ToggleButtonGroup>
        <Stack my={1}>
          <Stack my={1} gap={1} direction={'row'} alignItems={'center'} justifyContent={'center'}>
            <Typography variant={'h6'} textAlign={'center'}>Cogstruction </Typography>
            <Tooltip title={<>
              You can export your data and use it in <Link target={'_blank'}
                                                           underline={'always'}
                                                           color={'info.dark'}
                                                           href="https://github.com/automorphis/Cogstruction"
                                                           rel="noreferrer">Cogstruction</Link>
            </>}>
              <InfoIcon/>
            </Tooltip>
          </Stack>
          <Stack direction={'row'} gap={2}>
            <Button variant={'contained'} color={'primary'}
                    ...
                    onClick={() => handleCopy(state?.construction?.cogstruction?.cogData)}
...
const CogTooltip = ({ character, currentAmount, requiredAmount, cog }) => {
  return <>
    ...
    {currentAmount < requiredAmount ? <Typography>
      {kFormatter(currentAmount, 2)} / {kFormatter(requiredAmount, 2)} ({kFormatter(currentAmount / requiredAmount * 100, 2)}%)</Typography> : null}
