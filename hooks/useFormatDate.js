...
export const buildFormatString = ({ dateFormat = 'DMY', timeFormat = '24h', showSeconds = true, shortYear = false, timeOnly = false, dateOnly = false } = {}) => {
...
const useFormatDate = () => {
  ...
  const formatDate = (dateMs, options = {}) => {
    const { showSeconds = true, shortYear = false, timeOnly = false, dateOnly = false } = options;
    if (!isValid(new Date(dateMs))) return null;
    const formatString = buildFormatString({ dateFormat, timeFormat, showSeconds, shortYear, timeOnly, dateOnly });
