...
export const getStamps = (idleonData: any, account: any) => {
...
export const getStampsPerDay = (account: any) => {
  let stamps = 0;
...
export const evaluateStamp = (stamp: any, account: any, characters: any, gildedStamp = true, forcedStampReducer: any, forceMaxCapacity = false) => {
  ...
  if (stamp?.materials?.length > 0) {
    hasMaterials = checkHasMaterials(stamp?.materials, materialCost, account, false);
...
export const updateStamps = (account: any, characters: any, gildedStamp = true, forcedStampReducer?: any, forceMaxCapacity?: any) => {
...
const checkHasMaterials = (materials: any, materialCost: any, account: any, subtractGreenStacks: any) => {
...
const getGoldCostToLevel = (level: any, maxLevel: any, stamp: any, account: any) => {
...
const getGoldCost = (level: any, stamp: any, account: any) => {
...
export const getStampsBonusByEffect = (account: any, effectName: any, character?: any) => {
  return account?.stamps && Object.entries(account?.stamps)?.reduce((final: any, [stampTreeName, stampTree]: any) => {
    const foundStamps = stampTree?.filter(({ effect }: any) => effect?.includes(effectName));
...
export const getStampBonus = (account: any, stampTree: any, stampName?: any, character?: any) => {
  // In-game display is missing: pristine charm bonus, upgrade vault bonus
  const stampIndex = account?.stamps?.[stampTree]?.findIndex(({ rawName }: any) => rawName === stampName);
...
export const applyStampsMulti = (stamps: any, multiplier: any) => {
  return Object.entries(stamps).reduce((res, [stampCategory, stamps]: any) => {
    let updatedStamps = stamps;
    if (stampCategory !== 'misc') {
      updatedStamps = stamps?.map((stamp: any) => ({ ...stamp, multiplier }));
    }
    return { ...res, [stampCategory]: updatedStamps };
  }, {});
}

export const calcStampLevels = (allStamps: any) => {
  if (!allStamps) return 0;
  return Object.values(allStamps)?.reduce((res: any, stamps: any) => res + stamps?.reduce((stampsLevels: any, { level }: any) => stampsLevels + level, 0), 0);
};

export const calcStampCollected = (allStamps: any) => {
  if (!allStamps) return 0;
  return Object.values(allStamps)?.reduce((res: any, stamps: any) => res + stamps?.reduce((stampsCollected: any, { level }: any) => stampsCollected + (level > 0
    ? 1
    : 0), 0), 0)
}

export const unobtainableStamps = (['Stat_Wallstree_Stamp', 'SpoOoky_Stamp', 'Prayday_Stamp', 'Shiny_Crab_Stamp',
  'Talent_I_Stamp', 'Talent_V_Stamp', 'Gear_Stamp'] as any).toSimpleObject();