...
const parseVoteBallot = (idleonData, accountData) => {
  ...
  const companionBonus2 = isCompanionBonusActive(accountData, 19) ? accountData?.companions?.list?.at(19)?.bonus : 0;
  const companionBonus3 = isCompanionBonusActive(accountData, 41) ? accountData?.companions?.list?.at(41)?.bonus : 0;
  ...
  const cosmoBonus = getCosmoBonus({ majik: accountData?.hole?.holesObject?.idleonMajiks, t: 2, i: 3 });
  const winnerBonus = getWinnerBonus(accountData, '+{% Ballot Bonus');
  const equinoxBonus = getEquinoxBonus(accountData?.equinox?.upgrades, 'Voter_Rights');
