...
const shimmerIslandTrials: string[] = [
...
export const getIslands = (account: Account, characters: any[]) => {
  ...
  const preUnlockMultipliers: Record<number, number> = { 0: 0, 1: 8, 2: 32, 3: 80, 4: 200, 5: 500 };
...
const extraIslandDetails = (account: Account, characters: any[], index: number): any => {
  let result: any = {};
  if (index === 0) {
    const trash = (account as any)?.accountOptions?.[161];
    const items = [
      {
        icon: 'data/StampB47',
      ...
      },
      {
        ...
        acquired: getBribeBonus((account as any)?.bribes, 'The_Art_of_the_Bail')
      },
      {
        icon: 'data/Island1',
      ...
      },
      {
        ...
        acquired: characters?.some((char: any) =>
          char?.starTalents?.orderedTalents?.some(
            ({ maxLevel, name }: { maxLevel: number; name: string }) => name === 'FILTHY_DAMAGE' && maxLevel > 0
          )
        )
      },
      {
        ...
        acquired: (account as any)?.looty?.slabItems?.find(({
                                                     rawName,
                                                     obtained
    ...
    const bestDpsEver = notateNumber((account as any)?.accountOptions?.[172]);
    ...
    const hoursAfk = (account as any)?.accountOptions?.[184];
    result = {
      hoursAfk,
...
export const getIsland = (account: Account, islandName: string) => {
  return (account as any)?.islands?.list?.find(({ name }: { name: string }) => name === islandName);
}
