...
const Equipment = ({ equipment, tools, food, account, character, hideEmpty }) => {
  return <Stack>
    <Typography variant={'h5'}>Equipment</Typography>
    <Stack mt={2} direction={'row'} gap={1} flexWrap={'wrap'} justifyContent={'center'}>
      <EquipmentPage windowName={'equipment'} hideEmpty={hideEmpty} items={equipment?.slice(0, 8)} character={character}
                     account={account}/>
...
const EquipmentPage = ({ items, character, account, hideEmpty, windowName }) => {
  return <Box
    sx={{
      display: 'grid',
    ...
    {items?.map((item, itemIndex) => {
      ...
      const isEquipped = character?.[windowName]?.find(({ rawName: rName }) => rName === rawName);
      return itemIndex < 8 ?
